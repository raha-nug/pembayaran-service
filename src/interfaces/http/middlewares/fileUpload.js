...
export const uploadSingleFile = upload.single("buktiBayar");
...
export const ftpUploadMiddleware = async (req, res, next) => {
  ...
  const remoteFilename = `buktiBayar-${Date.now()}-${Math.round(
    Math.random() * 1e9
  )}${ext}`;
  ...
  req.uploadedFileUrl = `https://${process.env.FTP_HOST}/${
    process.env.FTP_REMOTE_DIR || "bukti_bayar"
  }/${remoteFilename}`;
  next();
};
